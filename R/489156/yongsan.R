...
#전체 페이지 찾기
page <- remDr$findElements(using = "css", '#searchForm > div.pagingWrap > p')#첫장
...
if(end_page>=2){
  for(i in 2:end_page){
    ...
    read <- remDr$findElements(using = "css", '#searchForm > ul > li:nth-child(1) > dl > dd.site > span')#자료소유 도서관명
    temp_lib<-sapply(read, function (x) {x$getElementText()})
    #temp_lib
