...
result_joongrang<-data.frame(unlist(temp_title), unlist(temp_lib), unlist(temp_ab))
result_joongrang
...
if(end_page>=2){
  for(i in 2:end_page){
    ...
    result_joongrang<-rbind(result_joongrang, data.frame(unlist(temp_title), unlist(temp_lib), unlist(temp_ab)))
